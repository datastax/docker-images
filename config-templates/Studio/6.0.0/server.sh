...
echo "Starting Studio. This may take a few minutes. You will be notified here when Studio is ready."
echo
echo
...
for include in  ./ target; do 
    ...
    if [ -r $server_jar ]; then
        ...
        for war in $include/lib/com.datastax.studio.ide.gremlin.web.dse_6_0_x-*.war; do
            GREMLIN_IDE_WAR_DSE_6_0_x=$war
        done
        ...
        for war in $include/lib/com.datastax.studio.ide.spark.sql.web*.war; do
            SPARK_SQL_IDE_WAR=$war
        done
...
exec $JAVA $STUDIO_JVM_ARGS -cp $SERVER_CLASSPATH $MAIN_CLASS $UI_DIR $API_WAR $CQL_IDE_WAR $SPARK_SQL_IDE_WAR $GREMLIN_IDE_WAR_DSE_6_0_x
